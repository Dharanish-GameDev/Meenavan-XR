...
namespace Unity.XRContent.Interaction
{
    ...
    /// </summary>
    public class ReelRotator : XRBaseInteractable
    {
        ...
        [SerializeField] private AudioSource reelRotatorSFX = null;
        ...
        void Start()
        {
            SetValue(m_Value);
            SetKnobRotation(ValueToRotation());
            previousRotation = m_Handle.rotation;
        }
        ...
        void StartGrab(SelectEnterEventArgs args)
        {
            ...
            reelRotatorSFX.enabled = true;
        }

        void EndGrab(SelectExitEventArgs args)
        {
            m_Interactor = null;
            reelRotatorSFX.enabled = false;
        }
        ...
        public override void ProcessInteractable(XRInteractionUpdateOrder.UpdatePhase updatePhase)
        {
            ...
            if (updatePhase == XRInteractionUpdateOrder.UpdatePhase.Dynamic)
            {
                ...
                {
                    UpdateRotation();
                    Quaternion currentRotation = m_Handle.rotation;
                    Quaternion rotationChange = Quaternion.Inverse(previousRotation) * currentRotation;
                    float angleChange = Quaternion.Angle(Quaternion.identity, rotationChange);

                    if (angleChange > rotationThreshold)
                    {
                        if (!reelRotatorSFX.isPlaying)
                            reelRotatorSFX.Play();
                    }
                    else
                    {
